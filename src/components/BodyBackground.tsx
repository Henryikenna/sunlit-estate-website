    ...
    <div className=" w-full h-[65dvh] lg:h-[80dvh]">
      <div className=" w-full h-full relative">
        <Image
          ...
          className="min-w-full min-h-full object-cover absolute brightness-75 -z-10"
          layout="fill"
        />

        <section className="relative flex flex-col justify-center items-center h-full text-center px-4 gap-[4.5rem] lg:px-0">
          <div className="">
            <h1 className=" font-racingSansOne text-white text-[2rem] md:text-[3rem] lg:text-[4rem]">
              Sunlit Caribbean Estates
            </h1>
            <h3 className=" font-openSans text-white font-semibold text-base md:text-xl lg:text-2xl">
              Find your dream home on the ABC Islands
            </h3>
          </div>

          {isSearchInputVisible === true ? (<label className="input input-bordered flex items-center w-full h-12 rounded-full gap-2 md:w-1/2 md:h-[3.9rem] lg:w-1/3 lg:h-[4.375rem]">
            <input
              type='search' name='query' id='default-search'
